...
export const generateActivitySuggestions = async (
  ...
  if (!GROQ_API_KEY) {
    console.warn('Groq API key not found, using fallback activities');
    return getFallbackActivities(city);
  }
  
  try {
    console.log('Generating activities for:', city, 'with API key:', GROQ_API_KEY.substring(0, 10) + '...');
...
No markdown, no explanation, just the JSON array.`
    ...
    if (!response.ok) {
      console.error('Groq API error response:', response.status, response.statusText);
      return getFallbackActivities(city);
    }
...
export const searchPlaces = async (
  ...
  if (!FOURSQUARE_API_KEY) {
    console.warn('Foursquare API key not found, using fallback places');
  ...
  try {
    ...
    console.log('Searching Foursquare for:', query);
    ...
    if (!response.ok) {
      console.error('Foursquare API error response:', response.status, response.statusText);
      return getFallbackPlaces(query);
    }
...
// Convert amount to INR with caching
let cachedRates: Record<string, number> | null = null;
let ratesCacheTime = 0;
const CACHE_DURATION = 60 * 60 * 1000; // 1 hour

export const convertToINR = async (amount: number, fromCurrency: string = 'USD'): Promise<{ inr: number; formatted: string }> => {
  try {
    // Return cached rates if still valid
    const now = Date.now();
    if (!cachedRates || now - ratesCacheTime > CACHE_DURATION) {
      cachedRates = await getCurrencyRates('USD');
      ratesCacheTime = now;
    }
    
    const inrRate = cachedRates['INR'] || 83.5; // Fallback rate
    let usdAmount = amount;
    
    // First convert to USD if not already
    if (fromCurrency !== 'USD' && cachedRates[fromCurrency]) {
      usdAmount = amount / cachedRates[fromCurrency];
    }
    
    // Convert USD to INR
    const inrAmount = Math.round(usdAmount * inrRate);
    ...
    console.error('INR conversion error:', error);
    // Fallback: approximate conversion
    const fallbackRate = 83.5;
    const inrAmount = Math.round(amount * fallbackRate);
    return {
      inr: inrAmount,
      formatted: `₹${inrAmount.toLocaleString('en-IN')}`
    };
...
// Format currency with INR conversion display
export const formatWithINR = (amount: number, originalCurrency: string = 'USD', inrRate: number = 83.5): string => {
  const inrAmount = Math.round(amount * inrRate);
  const symbol = getCurrencySymbol(originalCurrency);
  return `${symbol}${amount.toLocaleString()} (₹${inrAmount.toLocaleString('en-IN')})`;
};
